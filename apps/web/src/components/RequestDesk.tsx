...
export function RequestDesk() {
  ...
  async function reload(search = query) {
    setError('');
    const response = await fetch(`/api/v1/admin/requests?q=${encodeURIComponent(search)}`);
