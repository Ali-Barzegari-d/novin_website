...
import { ConvergenceMark } from './sections/ConvergenceMark';
import { StationPath } from './sections/StationPath';
import { ClosingMark } from './sections/ClosingMark';
...
const steps = [
  ['ثبت مسئله', 'از نیاز واقعی سازمان می‌گویید؛ بدون انتخاب یک پکیج.', 'رایگان', 0],
  ['بررسی و تماس', 'زمینه مسئله و امکان همکاری را در گفت‌وگو می‌سنجیم.', 'رایگان', 0],
  ['پیشنهاد جلسه', 'در صورت تناسب، دامنه، خروجی و مبلغ اختصاصی اعلام می‌شود.', 'پیش از پرداخت', 1],
  ['جلسه و جمع‌بندی', 'پس از وصول، جلسه هماهنگ و گزارش مقدماتی ارائه می‌شود.', 'خروجی مکتوب', 2],
  ['پروژه مستقل', 'ادامه کار در صورت توافق، با پیشنهاد و قرارداد جداگانه است.', 'توافق جدید', 3]
] as const;
...
export function HomeExperience() {
  return <>
        ...
        <div className="hero-composition">
          ...
          <aside className="hero-brief" aria-label="روش شروع همکاری">
            <figure className="hero-figure">
              <Image
                src="/images/financial-process-collage-hero.webp"
                alt="کلاژ تصویری مفهومی از کار تحلیل مالی: نمودارها، اسناد، ماشین‌حساب و هندسه ایرانی در پالت سرمه‌ای و فیروزه‌ای"
                width={900}
                height={675}
                sizes="(min-width: 1000px) 371px, calc(100vw - 48px)"
                priority
              />
              <figcaption><span className="figure-tag">تصویر مفهومی</span>از صورت‌بندی مسئله تا پذیرش نتیجه</figcaption>
            </figure>
            ...
            <div className="hero-path"><HeroPathArt /></div>
          </aside>
        </div>
    ...
    <section className="section journey-editorial"><div className="shell"><div className="section-heading-row"><div><p className="eyebrow">۰۴ / روش همکاری</p><h2>قدم بعدی، همیشه روشن.</h2></div><Link href="/process" className="text-link">جزئیات نحوه همکاری ←</Link></div><ol className="journey-track">{steps.map(([title, text, stage, phase], index) => <li key={title} data-phase={phase}><div className="journey-number">{(index + 1).toLocaleString('fa-IR').padStart(2, '۰')}</div><span className="journey-stage">{stage}</span><h3>{title}</h3><p>{text}</p></li>)}</ol></div></section>
    <section className="section proof-editorial"><div className="shell"><div className="section-spread"><div className="section-aside"><p className="eyebrow">۰۵ / شواهد همکاری</p><h2>مسئله. اقدام. نتیجه.</h2><p className="muted">هر سابقه واقعی، با همین سه فصل خوانا می‌ماند.</p><div className="aside-motif"><StationPath ariaLabel="ساختار روایت مطالعه موردی: مسئله، اقدام، نتیجه" stations={[{ title: 'مسئله' }, { title: 'اقدام' }, { title: 'نتیجه' }]} /></div></div><div className="proof-placeholder"><span className="document-label">در انتظار تأیید انتشار</span><h3>سوابق را با جزئیات قابل اتکا روایت می‌کنیم.</h3><p>مطالعات موردی این بخش پس از تأیید داخلی و دریافت مجوز کارفرما منتشر می‌شوند. تا آن زمان، نام، نشان یا آمار تأییدنشده‌ای نمایش نمی‌دهیم.</p><Link href="/projects" className="text-link">درباره مطالعات موردی ←</Link></div></div><div className="team-editorial"><div><p className="eyebrow">۰۶ / افراد پشت راه‌حل</p><h2>پیوند شناخت مالی و توان اجرا.</h2></div><div><p className="muted">معرفی اعضای تیم، نقش‌ها و سوابق، پس از دریافت اطلاعات واقعی و تأیید انتشار تکمیل می‌شود.</p><Link href="/about" className="text-link">درباره نوین ایرانیان ←</Link></div></div></div></section>
    <section className="closing-editorial"><div className="shell"><span className="closing-mark" aria-hidden="true"><ClosingMark /></span><div><p className="eyebrow">از یک گفت‌وگوی روشن شروع کنیم.</p><h2>مسئله را شما می‌شناسید.<br/>مسیر را با هم روشن می‌کنیم.</h2><p>بررسی اولیه رایگان است. جلسه کارشناسی، تنها پس از بررسی و با پیشنهاد اختصاصی ارائه می‌شود.</p><Link className="button button-primary" href="/request">ثبت مسئله و درخواست بررسی ←</Link></div></div></section>
  </>;
}
